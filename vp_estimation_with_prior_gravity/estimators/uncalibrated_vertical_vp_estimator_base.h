#ifndef UNCALIBRATED_VP_ESTIMATORS_UNCALIBRATED_VP_VERTICAL_ESTIMATOR_BASE_H_
#define UNCALIBRATED_VP_ESTIMATORS_UNCALIBRATED_VP_VERTICAL_ESTIMATOR_BASE_H_
...
class Uncalibrated_Vertical_VP_Estimator_Base: public VP_Estimator_Base {
public:
    ...
    Uncalibrated_Vertical_VP_Estimator_Base (
        const Eigen::Matrix4Xd& data, const Eigen::Vector3d& v,
        int ls_refinement = 0, int nms = 0, bool magsac_scoring = false): VP_Estimator_Base(data), v_(v), ls_refinement_(ls_refinement), nms_(nms), magsac_scoring_(magsac_scoring) {}
    ...
    void LeastSquares(const std::vector<int>& sample, ResultType* res) const {
        if (ls_refinement_ == 1)
            ...
            OrthogonalVerticalLeastSquares(sample, v_, data_, &(res->second), res->first, true); 
    }

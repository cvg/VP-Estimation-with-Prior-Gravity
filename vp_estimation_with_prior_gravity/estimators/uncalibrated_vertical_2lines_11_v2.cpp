...
int Uncalibrated_Vertical_2lines_11_V2::MinimalSolver(const std::vector<int>& sample, std::vector<ResultType>* res) const {
    ...
    std::vector<double> fs(2);
    std::vector<M3D> Rs(2);
    int num_sols = uncalibrated_vp_11_solver_v2(v_, ls.data(), Rs.data(), fs.data());
    if (num_sols == 0)
        return 0;
    res->resize(num_sols);

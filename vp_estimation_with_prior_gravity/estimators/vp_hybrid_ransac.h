#ifndef UNCALIBRATED_VP_ESTIMATORS_VP_HYBRID_RANSAC_H_
#define UNCALIBRATED_VP_ESTIMATORS_VP_HYBRID_RANSAC_H_
...
#include <estimators/sprt_preemptive_verification.h>
...
class VPHybridRansac : public HybridRansacBase {
 public:
  void set_sprt(bool sprt, const HybridSolver& solver,
                const ExtendedHybridLORansacOptions& options) {
    sprt_ = sprt;

    // Optionally initialize a SPRT preemptive verification module per solver
    if(sprt) {
      std::vector<std::vector<int>> min_sample_sizes;
      solver.min_sample_sizes(&min_sample_sizes);
      std::vector<std::vector<int>> minimal_sample(solver.num_data_types());
      ModelVector estimated_models;
      Sampler sampler(options.random_seed_, solver);
      sprts_.resize(5);
      for(size_t t = 0; t < 5; t++) {
        sampler.Sample(min_sample_sizes[t], &minimal_sample);
        sprts_[t] = new SPRTPreemptiveVerification<Model, ModelVector, HybridSolver>(
          solver, (int)t, minimal_sample, &estimated_models);
      }
    }
  }
  ...
  int EstimateModel(const ExtendedHybridLORansacOptions& options,
    ...
    if (options.final_least_squares_) {
      ...
      ScoreModel(options, solver, refined_model, kSqrInlierThresh,
                 kNumDataTypes, num_data, &score, stats.best_model_score);
 ...
 protected:
    ...
    *best_model_id = 0;
    ...
    for (int m = 0; m < num_models; ++m) {
      ...
      ScoreModel(options, solver, models[m], squared_inlier_thresholds,
                 num_data_types, num_data, &score, *best_score);
  ...
  void ScoreModel(const ExtendedHybridLORansacOptions& options,
                  ...
                  const std::vector<double>& squared_inlier_thresholds,
                  const int num_data_types, const std::vector<int> num_data,
                  double* score, double curr_best_score) {
    *score = 0.0;
    ...
    for (int t = 0; t < num_data_types; ++t) {
      if (options.data_type_weights_[t] < std::numeric_limits<double>::epsilon())
        continue;
      if(sprt_) {
        *score = sprts_[t]->verifyModel(solver, model, squared_inlier_thresholds[t],
          t, num_data, curr_best_score, options.data_type_weights_[t]);
      }
      else {
  ...
  inline double ComputeScore(const double squared_error,
                             const double squared_error_threshold) {
  ...
  void UpdateRANSACTerminationCriteria(
      ...
      std::vector<uint32_t>* max_iterations) {
  ...
  void LocalOptimization(const ExtendedHybridLORansacOptions& options,
    ...
    ScoreModel(options, solver, m_init, options.squared_inlier_thresholds_,
               kNumDataTypes, num_data, &score, *score_best_minimal_model);
    ...
    GetInliers(solver, m_init, squared_inlier_thresholds,
               &inliers_base);
    ...
    const int kNonMinSampleSize = 
        std::max(kMinNonMinSampleSize,
                 std::min(kMinNonMinSampleSize * options.non_min_sample_multiplier_,
                          static_cast<int>(inliers_base[0].size()) / 2));
    ...
    for (int r = 0; r < options.num_lo_steps_; ++r) {
      ...
      ScoreModel(options, solver, m_non_min,
                 options.squared_inlier_thresholds_, kNumDataTypes,
                 num_data, &score, *score_best_minimal_model);
  ...
  inline void UpdateBestModel(const double score_curr, const Model& m_curr,
                              ...
                              Model* m_best, int* best_solver_type) {
 ...
 protected:
  bool sprt_;
  std::vector<SPRTPreemptiveVerification<Model, ModelVector, HybridSolver>* > sprts_;
